...
def check_vk_response(response):
    if 'error' in response:
        raise ConnectionError(f'Возникла ошибка {response["error"]["error_msg"]}')
...
def get_link_to_upload(group_id, access_token, api_version=5.131):
    ...
    check_vk_response(decoded_server_details_response)
...
def upload_image(image_number_to_post, upload_link):
    ...
    check_vk_response(decoded_image_upload_response)
...
def save_image(server, photo, hash_, group_id, access_token, api_version=5.131):
    ...
    check_vk_response(decoded_image_save_response)
...
def post_image(owner_id, id_, group_id, comment, access_token, api_version=5.131):
    ...
    image_post_resonse = requests.post('http://api.vk.com/method/wall.post',
                  params=image_post_parameters)
    check_vk_response(image_post_resonse.json())
